...
router.get('/registerauction/:username', (req, res) => {
    let username = req.params.username;
    let devnet_link = '';
    ...
    client.connect(function(err) {
        ...
        db.collection('userdata').find({ username: username
        }).toArray(function(err, docs) {
            if (docs.length > 0) {
                let user = docs[0];
                let command = 'echo -n "' + user.wallet + '" > tmp.pem';

                exec(command, (error, stdout, stderr) => {
                    command  = 'bash -c "source ~/fob-project/smartcontract/contract/interactions/playground.sh && register_participant ./tmp.pem ' + username + '"';
                    exec(command, (error, stdout, stderr) => {
                        console.log(`${stdout}`);
                        console.log(`${stderr}`);
                        ...
                        exec('bash -c "rm tmp.pem"', (error, stdout, stderr) => {
                            res.render('newdigipet', {
                                username: username,
                                devnet_link: devnet_link,
                            });
...
router.get('/transferdigipet/:username', (req, res) => {
    command = 'bash -c "source ~/fob-project/smartcontract/contract/interactions/playground.sh && transfer_nft"';
    exec(command, (error, stdout, stderr) => {
        let lines = `${stderr}`.split('\n');
        let words = lines[4].split(':');
        let devnet_link = words[3].substring(1) + ':' + words[4];
...
router.post('/biddigipet/:username', (req, res) => {
    let egld = req.body.egld;
    let username = req.params.username;
    let devnet_link = '';
    ...
    client.connect(function(err) {
        ...
        db.collection('userdata').find({ username: username
            ...
            if (docs.length > 0) {
                ...
                exec(command, (error, stdout, stderr) => {
                    command  = 'bash -c "source ~/fob-project/smartcontract/contract/interactions/playground.sh && bid_participant ./tmp.pem ' + egld + '"';
                    exec(command, (error, stdout, stderr) => {
                        let lines = `${stderr}`.split('\n');
                        let words = lines[4].split(':');
                        devnet_link = words[3].substring(1) + ':' + words[4];
